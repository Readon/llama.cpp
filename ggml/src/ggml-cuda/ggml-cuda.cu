...
static void ggml_backend_cuda_buffer_set_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    if (tp_config && tp_config->enabled && ggml_cuda_tp_has_distributed_allocation(tensor)) {
        ...
        if (alloc_info && alloc_info->is_distributed) {
#ifndef NDEBUG
            GGML_LOG_DEBUG("%s: loading distributed tensor '%s' to GPU %d (allocated size: %.2f MiB)\n",
                   __func__, ggml_get_name(tensor), alloc_info->target_gpu_id, alloc_info->allocated_bytes / 1024.0 / 1024.0);
#endif
...
#ifndef NDEBUG
    GGML_LOG_DEBUG("%s: buffer allocation request: %.2f MiB on device %d\n", __func__, size / 1024.0 / 1024.0, target_device);
#endif
...
#ifndef NDEBUG
    GGML_LOG_DEBUG("%s: get_alloc_size for '%s': type=%d, ggml_nbytes=%.2f MiB, dimensions=[%ld x %ld]\n",
           __func__, ggml_get_name(tensor), tensor->type, size / (1024.0 * 1024.0), (long)tensor->ne[0], (long)tensor->ne[1]);
    GGML_LOG_DEBUG("%s: tensor details - ne[0]=%ld, ne[1]=%ld, ne[2]=%ld, ne[3]=%ld, nb[0]=%zu, nb[1]=%zu\n",
           __func__, (long)tensor->ne[0], (long)tensor->ne[1], (long)tensor->ne[2], (long)tensor->ne[3],
           tensor->nb[0], tensor->nb[1]);
    GGML_LOG_DEBUG("%s: tensor type_size=%zu, is_quantized=%s, tensor_ptr=%p\n",
           __func__, ggml_type_size(tensor->type), ggml_is_quantized(tensor->type) ? "yes" : "no", (const void*)tensor);
#endif
...
#ifndef NDEBUG
        GGML_LOG_DEBUG("%s: checking tensor '%s' for distributed allocation\n", __func__, ggml_get_name(tensor));
#endif
...
#ifndef NDEBUG
            GGML_LOG_DEBUG("%s: using distributed buffer size: %.2f MiB (original: %.2f MiB) for tensor %s\n",
                   __func__, distributed_size / 1024.0 / 1024.0, size / 1024.0 / 1024.0, ggml_get_name(tensor));
#endif
...
#ifndef NDEBUG
            GGML_LOG_DEBUG("%s: non-distributed tensor '%s' using original size: %.2f MiB\n",
                   __func__, ggml_get_name(tensor), size / 1024.0 / 1024.0);
#endif
            return size;
        }
    }
...
static bool ggml_cuda_compute_forward(ggml_backend_cuda_context & ctx, struct ggml_tensor * dst) {
    ...
    if (tp_config && tp_config->enabled) {
...
#ifndef NDEBUG
                    GGML_LOG_DEBUG("%s: enabling peer access between device %d and %d for tensor %s\n",
                           __func__, ctx.device, alloc_info->target_gpu_id, ggml_get_name(dst->src[i]));
#endif
        ...
        if (ggml_cuda_tp_has_distributed_allocation(dst)) {
            ...
            if (alloc_info && alloc_info->is_distributed && alloc_info->target_gpu_id != ctx.device) {
#ifndef NDEBUG
                GGML_LOG_DEBUG("%s: enabling peer access between device %d and %d for dst tensor %s\n",
                       __func__, ctx.device, alloc_info->target_gpu_id, ggml_get_name(dst));
#endif
        ...
        case GGML_OP_ADD1: // TODO: more efficient implementation
            {
                ...
                if (tp_config && tp_config->enabled) {
                    ...
                    if (has_distributed) {
#ifndef NDEBUG
                        GGML_LOG_DEBUG("%s: ADD operation with distributed tensors - skipping for now to avoid crash\n", __func__);
#endif
        ...
        case GGML_OP_MUL:
            {
                ...
                if (tp_config && tp_config->enabled) {
                    ...
                    if (has_distributed) {
#ifndef NDEBUG
                        GGML_LOG_DEBUG("%s: MUL operation with distributed tensors - skipping for now to avoid crash\n", __func__);
#endif
        ...
        case GGML_OP_RMS_NORM:
            {
                ...
                if (tp_config && tp_config->enabled) {
                    ...
                    if (has_distributed) {
#ifndef NDEBUG
                        GGML_LOG_DEBUG("%s: RMS_NORM operation with distributed tensors - skipping for now to avoid crash\n", __func__);
#endif
        ...
        case GGML_OP_MUL_MAT:
            // Handle tensor parallelism for MUL_MAT operations
            {
                ...
                if (tp_config && tp_config->enabled) {
                    ...
                    if (has_distributed) {
#ifndef NDEBUG
                        GGML_LOG_DEBUG("%s: MUL_MAT operation with distributed tensors - skipping for now to avoid crash\n", __func__);
#endif
